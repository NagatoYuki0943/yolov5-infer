...
class Inference(ABC):
    ...
    def nms(self, detections_in: np.ndarray) -> list[np.ndarray]:
        ...
        Args:
            detections_in (np.ndarray): 检测到的数据 [b, 25200, 85]
        ...
        for detection_in in detections_in:
            # 通过置信度过滤一部分框
            detection_k    = detection_in[detection_in[..., 4] > self.confidence_threshold]
            # 位置坐标
            loc            = detection_k[..., :4]
            # 置信度
            confidences    = detection_k[..., 4]
            # 分类
            cls            = detection_k[..., 5:]
            # 最大分类index
            ...
            # 置信度
            confidences    = confidences[max_cls_score > self.confidence_threshold]
            # 类别index
    ...
    def to_origin_size(self, detection_in: np.ndarray, ratio: float, origin_shape: np.ndarray) -> np.ndarray:
        ...
        Args:
            detection_in (np.ndarray): np.float32
                    [
                        [class_index, confidence, xmin, ymin, xmax, ymax],
                        ...
                    ]
    ...
    def single(
        ...
        # 3. NMS
        t3 = time.time()
        nms_results = self.nms(results)    # [1, 25200, 85] -> [[[class_index, confidences, xmin, ymin, xmax, ymax],]]
